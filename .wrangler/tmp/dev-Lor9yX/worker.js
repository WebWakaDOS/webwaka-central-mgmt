...
// .wrangler/tmp/bundle-x4GfrB/strip-cf-connecting-ip-header.js
...
__name(PlatformLogger, "PlatformLogger");
...
// src/modules/ai-billing/core.ts
async function processAIUsageEvent(env2, tenantId, payload, eventId) {
  const now = Date.now();
  ...
  const DEFAULT_MONTHLY_TOKENS = 1e6;
  ...
  if (!existing) {
    ...
    if (!payload.usedByok && payload.estimatedCostUsd && payload.estimatedCostUsd > 0) {
      const costKobo = Math.round(payload.estimatedCostUsd * 1650 * 100);
      if (costKobo > 0) {
        await env2.DB.prepare(
        ...
        await emitBillingEvent(env2, tenantId, {
...
__name(processAIUsageEvent, "processAIUsageEvent");
async function emitBillingEvent(env2, tenantId, event) {
  const key = `event:${Date.now()}:${crypto.randomUUID()}`;
  const body = JSON.stringify(event);
  if (env2.EVENTS) {
    try {
      await env2.EVENTS.put(key, body, { expirationTtl: 86400 });
    } catch (err) {
      console.warn(`[ai-billing] Failed to write billing event to KV: ${err}`);
    }
  }
  if (env2.EVENT_BUS_URL) {
    try {
      await fetch(env2.EVENT_BUS_URL, {
        method: "POST",
...
// .wrangler/tmp/bundle-x4GfrB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-x4GfrB/middleware-loader.entry.ts
