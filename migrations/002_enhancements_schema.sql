...
CREATE INDEX IF NOT EXISTS idx_ai_usage_tenant     ON ai_usage_ledger(tenant_id);
CREATE INDEX IF NOT EXISTS idx_ai_usage_capability ON ai_usage_ledger(capability_id);
CREATE INDEX IF NOT EXISTS idx_ai_usage_recorded   ON ai_usage_ledger(recorded_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_ai_quota_tenant ON ai_quota_ledger(tenant_id);
...
CREATE INDEX IF NOT EXISTS idx_dlq_status     ON webhook_dlq(status);
CREATE INDEX IF NOT EXISTS idx_dlq_next_retry ON webhook_dlq(next_retry_at);
CREATE INDEX IF NOT EXISTS idx_dlq_tenant     ON webhook_dlq(tenant_id);
CREATE INDEX IF NOT EXISTS idx_dlq_created    ON webhook_dlq(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_fraud_event_id ON fraud_scores(event_id);
CREATE INDEX IF NOT EXISTS idx_fraud_tenant   ON fraud_scores(tenant_id);
CREATE INDEX IF NOT EXISTS idx_fraud_risk     ON fraud_scores(risk_level);
CREATE INDEX IF NOT EXISTS idx_fraud_created  ON fraud_scores(created_at DESC);

-- ─── Tenant Suspension Log ────────────────────────────────────────────────────
-- Immutable audit trail of every tenant suspend / unsuspend action.
CREATE TABLE IF NOT EXISTS tenant_suspension_log (
  id           TEXT PRIMARY KEY,
  tenant_id    TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_susp_tenant  ON tenant_suspension_log(tenant_id);
CREATE INDEX IF NOT EXISTS idx_susp_created ON tenant_suspension_log(created_at DESC);
...
INSERT OR IGNORE INTO fx_rates (id, currency, rate_to_ngn, updated_at) VALUES
  ('fx_ngn', 'NGN', 1.0,  0),
  ('fx_ghs', 'GHS', 90.0, 0),
  ('fx_kes', 'KES', 5.5,  0);
