...
--
-- WCM-001: Adds cryptographic hash chaining columns to cmgt_ledger_entries for immutability
-- WCM-003: Introduces the cmgt_affiliates table for the multi-level commission engine
...
-- Add hash chain columns to cmgt_ledger_entries.
...
ALTER TABLE cmgt_ledger_entries ADD COLUMN previous_hash TEXT NOT NULL DEFAULT 'GENESIS';
ALTER TABLE cmgt_ledger_entries ADD COLUMN entry_hash    TEXT NOT NULL DEFAULT '';

-- ─── WCM-003: Affiliate Engine ────────────────────────────────────────────────
-- Stores the multi-level affiliate hierarchy (up to 5 levels deep).
CREATE TABLE IF NOT EXISTS cmgt_affiliates (
  id              TEXT PRIMARY KEY,                   -- e.g. 'aff_<uuid>'
  user_id         TEXT NOT NULL,                      -- platform user or tenant ID
  parent_id       TEXT,                               -- NULL for top-level cmgt_affiliates
...
CREATE INDEX IF NOT EXISTS idx_aff_user_id   ON cmgt_affiliates(user_id);
CREATE INDEX IF NOT EXISTS idx_aff_parent_id ON cmgt_affiliates(parent_id);
CREATE INDEX IF NOT EXISTS idx_aff_status    ON cmgt_affiliates(status);

-- Commission payouts ledger — records each calculated commission split
CREATE TABLE IF NOT EXISTS cmgt_affiliate_commissions (
  id              TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_aff_comm_affiliate ON cmgt_affiliate_commissions(affiliate_id);
CREATE INDEX IF NOT EXISTS idx_aff_comm_txn       ON cmgt_affiliate_commissions(transaction_id);
CREATE INDEX IF NOT EXISTS idx_aff_comm_status    ON cmgt_affiliate_commissions(status);
CREATE INDEX IF NOT EXISTS idx_aff_comm_created   ON cmgt_affiliate_commissions(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_ledger_acct_time
  ON cmgt_ledger_entries(account_id, created_at DESC);

-- (account_type, created_at): summary queries group by type
CREATE INDEX IF NOT EXISTS idx_ledger_type_time
  ON cmgt_ledger_entries(account_type, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_cme_velocity
  ON cmgt_central_mgmt_events(tenant_id, event_type, received_at DESC);
